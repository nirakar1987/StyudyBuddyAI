...
import { startChat, continueChat, analyzeHandwrittenWork, generateContentWithSearch } from '../services/geminiService';
...
const LessonView: React.FC<LessonViewProps> = ({ context }) => {
    ...
    const initializeChat = useCallback(async () => {
        ...
        startChat(studentProfile!);
        try {
            const response = await continueChat(`Start a lesson on ${studentProfile!.subject} for a grade ${studentProfile!.grade} student.`);
            handleAiResponse({ role: 'model', parts: [{ text: response.text }] });
        } catch (e: any) {
            handleAiResponse({ role: 'model', parts: [{ text: e.message || "I'm having trouble starting our lesson. Please try again in a moment." }] });
        }
    }, [handleAiResponse, studentProfile, setAvatarState]);
    ...
    }, [initializeChat, chatHistory.length]);
    ...
    }, [inputValue, transcript, setChatHistory, setAvatarState, setScore, handleAiResponse, useSearch, chatHistory]);
    ...
    return (
        <>
            ...
            <div className="flex flex-col h-full max-h-[75vh] animate-fade-in">
                <h2 className="text-2xl font-bold text-[var(--color-primary)] mb-4 border-b border-[var(--color-border)] pb-2">Lesson: {studentProfile!.subject}</h2>
                                        ...
                                        return (
                                            <div key={i} className="text-sm leading-relaxed space-y-2">
                                                {part.text.split('\n').map((line, lineIdx) => {
                                                    ...
                                                    if (line.startsWith('### ')) {
                                                        return <h3 key={lineIdx} className="font-bold text-lg mt-3 mb-1 text-indigo-300">{line.replace('### ', '')}</h3>;
                                                    }
                                                    // Handle bold headings/lines (**Heading**)
                                                    if (line.trim().startsWith('**') && line.trim().endsWith('**') && line.length < 50) {
                                                        return <div key={lineIdx} className="font-bold mt-3 mb-1 text-indigo-200">{line.replace(/\*\*/g, '')}</div>;
                                                    }
                                                                ...
                                                                <span>
                                                                    {line.replace(/^[\-\*]\s+/, '').split(/(\*\*.*?\*\*)/).map((chunk, cIdx) =>
                                                                        chunk.startsWith('**') && chunk.endsWith('**')
                                                                            ? <strong key={cIdx} className="text-indigo-200">{chunk.replace(/\*\*/g, '')}</strong>
                                                                            : chunk
                                                                    )}
                                                                </span>
                                                            </div>
                                                    ...
                                                    if (/^\d+\.\s/.test(line.trim())) {
                                                        return (
                                                            <div key={lineIdx} className="flex gap-2 ml-1">
                                                                <span className="font-bold min-w-[1.2em] text-indigo-400">{line.match(/^\d+\./)?.[0]}</span>
                                                                <span>
